...
- **En Fabric:** puedes definirla al inicio del notebook con `os.environ["ENTSOE_TOKEN"] = "<tu-token>"` en una celda separada que **no subas al repositorio** (o usa Azure Key Vault, ver [Decisiones técnicas](#6-gestión-del-token-de-entso-e)), o configúrala como variable de entorno del entorno de Spark del workspace.
